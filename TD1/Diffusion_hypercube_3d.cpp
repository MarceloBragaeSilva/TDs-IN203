...
int main(int nargs, char* argv[])
{
	...
	if (rank == 0) {
		...
		MPI_Send(&jeton, 1, MPI_INT, 4, tag, MPI_COMM_WORLD);
	}
	if (rank == 1) {
		MPI_Recv(&jeton, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
		MPI_Send(&jeton, 1, MPI_INT, 3, tag, MPI_COMM_WORLD);
		MPI_Send(&jeton, 1, MPI_INT, 5, tag, MPI_COMM_WORLD);
	}
	if (rank == 2) {
		MPI_Recv(&jeton, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
		MPI_Send(&jeton, 1, MPI_INT, 6, tag, MPI_COMM_WORLD);
	}
	if (rank == 3) {
		MPI_Recv(&jeton, 1, MPI_INT, 1, tag, MPI_COMM_WORLD, &status);
		MPI_Send(&jeton, 1, MPI_INT, 7, tag, MPI_COMM_WORLD);
	}
	if (rank == 4) {
		MPI_Recv(&jeton, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
	}
	...
	if (rank == 6) {
		MPI_Recv(&jeton, 1, MPI_INT, 2, tag, MPI_COMM_WORLD, &status);
	}
	if (rank == 7) {
		MPI_Recv(&jeton, 1, MPI_INT, 3, tag, MPI_COMM_WORLD, &status);
	}
	if (rank < 8)
