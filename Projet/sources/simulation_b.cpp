...
void simulation(bool affiche)
{
    ...
    while (!quitting)
    {
        ...
        output << jours_ecoules << "\t" << grille.nombreTotalContaminesGrippe() << "\t"
            << grille.nombreTotalContaminesAgentPathogene() << std::endl;
        if (jours_ecoules > 365) quitting = true;   
        jours_ecoules += 1;
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Temps Simulation : " << elapsed_seconds.count() << std::endl;
...
/*
2.1 Mesure du temps
Pour une simulation d'une annee, 365 jours
Temps avec l'affichage : 25.82 secondes
Temps sans l'affichage : 6.25 secondes "./simulation.exe -nw"

2.2
segmentation fault lorsque on modifie m_statistiques de privé à public ????
besoin de creer un nouveau methode getStatistiques
Temps avec l'affichage : 18.9 secondes "mpirun -np 2 simulation.exe"
Temps sans l'affichage : 6.46 secondes "mpirun -np 2 simulation.exe -nw"
...
Comme espéré, le programme utilise seulement 2 coeurs, un pour afficher la
simulation et autre pour la simulation proprement dit. On voit que la division
de taches ont résulté une meilleure de performance, même si le calcule de la
simulation a peu changé quand on éxécute sans l'affichage.
