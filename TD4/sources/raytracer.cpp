...
void render(const std::vector<Sphere> &spheres, unsigned width = 640, unsigned height = 480)
{
    ...
    // Trace rays
    int grain_size = 100;
    tbb::parallel_for(tbb::blocked_range2d<unsigned, unsigned>(0, width, grain_size, 0, height, grain_size),
    ...
    std::ofstream ofs("./image_tbb.ppm", std::ios::out | std::ios::binary);
    ofs << "P6\n" << width << " " << height << "\n255\n";
