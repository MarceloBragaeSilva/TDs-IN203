...
int main(int nargs, char* argv[]) {
    ...
    if (rank == 0) {  //Tache Maitre
        ...
        while (count_task < nb_tasks) {//pixels.data()+W*count_task
            MPI_Recv(pixels.data()+W*(count_task-nbp+1), W, MPI_INT, MPI_ANY_SOURCE, tag, globComm, &status);
            std::cout << "[" << count_task - nbp + 2 << "/" << H << "]\n";


            MPI_Send(&count_task, 1, MPI_INT, status.MPI_SOURCE, tag, globComm);
            count_task++;
        }
       // while (count_task-nbp+1 < H) { //wait for last 'nbp' tasks to finish
          //  MPI_Recv(pixels.data() + W * (nb_tasks-nbp+count_task), W, MPI_INT, MPI_ANY_SOURCE, tag, globComm, &status);
            //std::cout << "[" << count_task - nbp + 2 << "/" << H << "]\n";
           // count_task++;
       // }
        count_task = -1;
        ...
        savePicture("mandelbrot_maitre_esclave.tga", W, H, pixels, maxIter);
        std::cout << "\nPicture saved\n";
    }
    else {  //Tache esclave
        ...
        while (num_task != -1) {
            ...
            if (num_task >= 0) {
                //execute ligne 'num_task' de l'image
                //std::cout << rank << " execute tache " << num_task << "\n";
                computeMandelbrotSetRow(W, H, maxIter, num_task, pixels_ligne.data());
                MPI_Send(pixels_ligne.data(), W, MPI_INT, 0, tag, globComm);
            }
        }
    }
